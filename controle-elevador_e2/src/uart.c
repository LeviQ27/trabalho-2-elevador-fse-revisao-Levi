...
{
    pthread_mutex_lock(&e2_uartMutex);
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (count <= 0)
        {
            perror("(Botão e2): Erro durante a transmissão UART\n");
            return;
        }
    }
    else
    {
        perror("(Botão e2): Erro - A porta UART não foi inicializada\n");
        return;
    }
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (rx_length < 0)
        {
            perror("(Botão e2): Erro na leitura UART\n");
        }
    ...
    pthread_mutex_unlock(&e2_uartMutex);
}
...
int e2_captaEncoder()
{
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (count <= 0)
        {
            printf("(Encoder e2): UART TX error\n");
        }
    }
    tcflush(uart0_filestream, TCIFLUSH);
    delay(50);
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (rx_length < 0)
        {
            printf("(Encoder e2): Erro na leitura encoder.\n");
        }
        else if (rx_length > 0)
        {
...
void e2_botaoApaga(int base)
{
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (count <= 0)
        {
            printf("(Apaga Botao e2): UART TX error\n");
        }
    }
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (rx_length < 0)
        {
            printf("(Apaga Botao e2): Erro na leitura.\n");
        }
        else if (verificaCrc(rx_buffer, rx_length))
        {
        }
...
void e2_pwmEnvia(int pwm)
{
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (rx_length < 0)
        {
            perror("(PWM e2): Erro na leitura UART\n");
        }
        else if (verificaCrc(rx_buffer, rx_length))
        {
        }
    ...
    {
        perror("(PWM e2): Erro - A porta UART não foi inicializada\n");
    }
    pthread_mutex_unlock(&e2_uartMutex);
}
...
void e2_temperaturaEnvia(float temperatura)
{
    ...
    if (uart0_filestream != -1)
    {
        ...
        if (rx_length < 0)
        {
            perror("(Temp e2): Erro na leitura UART\n");
        }
        else if (verificaCrc(rx_buffer, rx_length))
        {
        }
    ...
    {
        perror("(Temp e2): Erro - A porta UART não foi inicializada\n");
    }
